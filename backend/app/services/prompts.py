...
def build_answer_prompt(
    *,
    ...
    return (
        ...
        "- Always make actor and action explicit: write as \"[character name] did ...\".\n"
        "- Never answer in first person (e.g., 私は / 僕は / I / we).\n"
        "- If the player asks \"you\", reinterpret it as asking about relevant characters and answer with names.\n"
        "- After the answer body, append a follow-up block with exact tags.\n"
        "- Use exact opening tag: <FOLLOW_UP_QUESTIONS> and closing tag: </FOLLOW_UP_QUESTIONS>.\n"
        "- Inside that block, output exactly 3 lines in this format: Q1: ..., Q2: ..., Q3: ...\n"
        "- Each follow-up should be a concrete next question the player can ask immediately.\n"
        "- Make the 3 follow-up questions strategically reduce uncertainty about: killer, motive, method, and trick.\n"
        "- The UI already always shows generic defaults (who saw victim last / where were you / conflict with victim).\n"
        "- Do not repeat those generic defaults unless strictly necessary; propose context-aware next steps.\n"
...
def build_contradiction_prompt(
    *,
    ...
    return (
        ...
        "Also set contradiction=true when actor/subject is ambiguous "
        "(e.g., first-person reply without explicit character names).\n"
        "In fixed_answer, rewrite so every key action has an explicit character name.\n"
        f"CASE_JSON: {case_json}\n"
        f"Question: {question}\n"
        f"ANSWER: {answer}"
    )
